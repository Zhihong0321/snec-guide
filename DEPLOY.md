...
| `UNIAPI_KEY` | Yes* | Gemini chat model and **image analysis** for MiniMax M2.7 attachments |
...
\*Image attachments on MiniMax M2.7: chat uses `OPENAI_API_KEY` (OKAI); images are analyzed via **UniAPI Gemini** (`UNIAPI_KEY`, optional `IMAGE_VISION_MODEL`). Run `python scripts/verify_image_chat.py` after deploy.
